...
public class PaymentGUI extends JPanel {
		...
		payButton.addActionListener(new ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				...
				try {
					...
					boolean creditCardPayment = creditCardRadioButton
							.isSelected();
					boolean cash = cashRadioButton.isSelected();
					if (creditCardPayment) {

						String creditCard = Utilities
								.handleNullPointerException(
										creditCardTextFieldOne.getText())
								.trim()
								+ Utilities
						...
						InUserGUI.attendantBo.updateEndDate(InUserGUI.ticketNumberTextArea
								.getText());
						...
						int numberofAvailableCars = InUserGUI.adminBo
								.getNumberOfAvailableSlots();

						InUserGUI.numberofAvailableCars
								.setText("Number of Available Lots = "
			...
			private void validateCreditCard(String creditCard) throws RemoteException{
				...
				if (!isEmpty) {
					if (validate.aValidNumber(creditCard)) {
						...
						if (validateOtherDetails) {
							InUserGUI.attendantBo
									.updateEndDate(InUserGUI.ticketNumberTextArea
											.getText());
							...
							boolean isCreditCardExists = InUserGUI.attendantBo
									.checkCreditCardExists(creditCard, month,
											year, cvv, amountToDeduct);
							if (isCreditCardExists) {
								InUserGUI.dispose();
								InUserGUI.attendantBo.updateBalance(creditCard, month,
										year, cvv, amountToDeduct);
								JOptionPane.showMessageDialog(null,
										"Payment Successful !!!");
								String[] strings = null;
								InUserGUI.main(strings);

								int numberofAvailableCars = InUserGUI.adminBo.getNumberOfAvailableSlots();
								InUserGUI.numberofAvailableCars
										.setText("Number of Available Lots = "
												+ numberofAvailableCars);
							}
	...
	private void setNames() {
		...
		creditCardTextFieldOne
				.setName(ComponentNames.CREDIT_CARD_TEXT_FIELD_ONE);

		creditCardTextFieldTwo
				.setName(ComponentNames.CREDIT_CARD_TEXT_FIELD_TWO);
		creditCardTextFieldThree
				.setName(ComponentNames.CREDIT_CARD_TEXT_FIELD_THREE);
		creditCardTextFieldFour
				.setName(ComponentNames.CREDIT_CARD_TEXT_FIELD_FOUR);

		expiryDateTextFieldOne
				.setName(ComponentNames.EXPIRY_DATE_TEXT_FIELD_ONE);

		expiryDateTextFieldTwo
				.setName(ComponentNames.EXPIRY_DATE_TEXT_FIELD_TWO);
