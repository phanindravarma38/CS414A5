		...
		try {
			...
			if (customerNameTextArea != null
				...
				if (customerNameTextArea.getText().split(" ").length > 1)
					customer.setLastname(customerNameTextArea.getText().split(
							" ")[1]);

			}
			customer.setVehicle(vehicle);
			int ticketNumber = InUserGUI.attendantBo.issueTicket(customer, attendant);
			if (ticketNumber == 0) {
				...
				numberOfAvailableCars.setText("Number of Available Lots = "
						+ InUserGUI.adminBo.getLotCapacity());
			}
			if (InUserGUI.numberOfAvailableSpaces <= 0) {
